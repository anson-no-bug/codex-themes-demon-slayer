...
## 2026-07-22 直启方案恢复与安装器补强

- GitHub 主线恢复为 b-nnett 直补丁架构：安装后直接打开官方 Codex，不依赖单独的 Codex++ 启动器。
- 新增无 Git 安装器；缺少 b-nnett CLI 时优先使用 Homebrew，或在已有 Node.js 20+ / npm 时调用 b-nnett 官方源码安装器。
- 强制使用 `install --local` / `repair --force --local`，避免 adhoc 签名引发 `better-sqlite3` Team ID 不一致。
- `npm run build`、`npm run check`、`npm run check:install`、b-nnett `validate-tweak` 与完整 `runtime-qa` 均通过；QA 复用系统 Chrome，不要求 Playwright 另行下载 Chromium。
